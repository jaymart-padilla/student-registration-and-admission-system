...
if (isset($_POST['submit'])) {
  ...
  if (empty($formError)) {
    ...
    $stmt = mysqli_prepare(
      $conn,
      "UPDATE tbluser SET FirstName = ?, LastName = ? WHERE ID = ? AND Privilege = 'admin'"
    );
    if ($stmt) {
      mysqli_stmt_bind_param($stmt, "ssi", $fName, $lName, $adminid);
        ...
        <div class="container-fluid">
          ...
          <div class="row" style="overflow: auto;">
            ...
            <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" name="submit" method="post" enctype="multipart/form-data" class="php-email-form">
              <?php
              $adminid = $_SESSION['id'];
              $ret = mysqli_query($conn, "SELECT * FROM tbluser WHERE ID='$adminid' AND Privilege = 'admin'");
              $cnt = 1;
                          ...
                          <div class="card-body">
                            ...
                            <div class="row">
                              ...
                              <div class="col-xl-12 col-lg-12">
                                <fieldset>
                                  <h6>Admin Regestration date </h6>
                                  <div class="form-group">
                                    <input class="form-control white_bg" id="createdAt" name="createdAt" value="<?php echo $row['CreatedAt']; ?>" readonly='true' type="text" required>
                                  </div>
                                </fieldset>
                              </div>
